                ...
                <div class="col">
                    <div class="row">
                        <div class="col">
                            ...
                            <%
                                ...
                                int j = 1;
                                if (rol.equals("Invited")) {
                                    out.println("<br><br><br><br><h3>No puede modificar ni eliminar datos porque no tiene los privilegios necesarios</h3>");
                                } else {
                            %>
                            <form method="post">
                                <div class="row justify-content-center">
                                    <div class="input-group mb-3 ajust-select fondo">
                                        ...
                                        <select class="custom-select fondo-submenu boton-desplegable letras-boton " id="inputGroupSelect02" name="combo" method="post">
                            ...
                            if (!opc.equals("Select")) {
                                try {
                                    ...
                                    String query = "SELECT curso.NombreCurso AS \"Nombre de Curso\", periodo.FechaInicio AS \"Fecha de inicio\", "
                                            ...
                                            + "JOIN periodo ON curso.IDCurso=periodo.IDCurso WHERE escuela.Siglas=\""
                                            + opc + "\" ORDER BY MONTH(fechaInicio);";
                                    String nombreCurso = request.getParameter("caja1");
                                    ResultSet rs = consulta.executeQuery(query);
                                    ResultSetMetaData rsmd = rs.getMetaData();
                                    int columnCount = rsmd.getColumnCount();
                                    out.println("<form method=\"POST\">");
                                    out.println("<div><center><table class=\"table-hover fondo-tabla\"><tr>");
                                    ...
                                    while (rs.next()) {
                                        ...
                                        for (int i = 1; i <= columnCount; i++) {
                                            ...
                                            if (i == 1) {
                                                out.println("<td><input name='caja" + i + "' value='" + columnValue + "' onload='caja1(" + j + ")' disabled></td>");
                                            } else if (i == 2) {
                                                out.println("<td><input name='caja" + i + "' value='" + columnValue + "' onload='caja1(" + j + ")' disabled></td>");
                                            } else if (i == 6) {
                                                out.println("<td><input name='caja" + i + "' value='" + columnValue + "' onload='caja1(" + j + ")' disabled></td>");
                                            } else {
                                                out.println("<td><input name='caja" + i + "' value='" + columnValue + "'></td>");
                                            }
                                        }
                    %>
                    <td><input type="button" onclick="modificarBaseDeDatos(<%=j%>);" id="boton<%=j%>" style="font-size: 12pt;" value="Modificar"></input></td>
                    <td><input type="button" onclick="eliminarBaseDeDatos(<%=j%>);" id="boton<%=j%>" style="font-size: 12pt;" value="Eliminar"></input></td>
                    ...
                    <script>
                        window.onload = function () {
                            var numRows = <%= j - 1%>; // Assuming j is the total number of rows
